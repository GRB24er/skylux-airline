...
export async function POST() {
  try {
    ...
    if (count > 50) return NextResponse.json({ skip: true, message: `Already have ${count} flights` });

    const ac = await Aircraft.find({ type: "commercial" }).limit(2);
    if (!ac.length) return NextResponse.json({ success: false, error: "No aircraft found" }, { status: 400 });
    ...
    for (let day = 0; day < 14; day++) {
      for (const r of routes) {
        for (let t = 0; t < 3; t++) {
          ...
          dep.setHours(times[t].h, times[t].m + Math.floor(Math.random()*15), 0, 0);
          ...
          flights.push({
            flightNumber: `SX ${100 + Math.floor(Math.random() * 900)}`,
            type: "commercial", airline: "SKYLUX Airways", isActive: true,
