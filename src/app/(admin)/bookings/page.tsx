...
export default function BookingsPage() {
  ...
  const sendBookingEmail=async(bookingId:string,type:string)=>{
    ...
    try{
      ...
      setEmailMsg(d.success?`? ${d.message}`:`? ${d.error||"Failed"}`);
  ...
  const sendCustomEmail=async()=>{
    ...
    try{
      ...
      const d=await res.json();
      setEmailMsg(d.success?"? Sent!":"? Failed");
    ...
    }catch{setEmailMsg("? Network error");setSendingEmail(null)}
  };
  ...
  const btn=(label:string,color:string,onClick:()=>void,disabled?:boolean)=>(
    <button onClick={onClick} disabled={disabled} style={{padding:"4px 10px",borderRadius:6,border:`1px solid ${color}25`,background:disabled?`${color}08`:"transparent",color:disabled?`${color}60`:color,fontSize:10,fontWeight:600,cursor:disabled?"wait":"pointer",fontFamily:"inherit",whiteSpace:"nowrap",transition:"all 0.15s"}}
      onMouseEnter={e=>{if(!disabled)(e.target as any).style.background=color+"15"}}
      onMouseLeave={e=>{if(!disabled)(e.target as any).style.background="transparent"}}
    >{label}</button>
  );
  ...
  return (
    <div style={{padding:"28px 32px",fontFamily:"'Plus Jakarta Sans',system-ui,sans-serif",color:C.text}}>
      ...
      {emailMsg&&!showEmailModal&&<div style={{padding:"10px 16px",marginBottom:14,borderRadius:10,background:emailMsg.startsWith("?")?C.emerald+"12":C.hot+"12",border:`1px solid ${emailMsg.startsWith("?")?C.emerald:C.hot}25`,fontSize:13,color:emailMsg.startsWith("?")?C.emerald:C.hot,fontWeight:600}}>{emailMsg}</div>}
        ...
        {filtered.length===0?(<div style={{padding:"60px 0",textAlign:"center",color:C.textDim}}><div style={{fontSize:36,marginBottom:12}}>??</div><div style={{fontSize:15,fontWeight:600}}>{bookings.length===0?"No bookings yet":"No matching bookings"}</div></div>):(
          <table style={{width:"100%",borderCollapse:"collapse"}}>
            ...
            <tbody>{filtered.map((b:any,i:number)=>{
              ...
              return (<>
              <tr key={b._id} style={{borderBottom:`1px solid ${C.glassBorder}08`,animation:`fadeUp 0.3s ease-out ${i*0.02}s both`,cursor:"pointer"}}
                onClick={()=>setExpandedBooking(isExpanded?null:b._id)}
                onMouseEnter={(e:any)=>e.currentTarget.style.background="rgba(255,255,255,0.015)"}
                ...
                <td style={{padding:"12px 14px",fontSize:13}}>{b.passengers?.[0]?.firstName} {b.passengers?.[0]?.lastName}{b.passengers?.length>1?` +${b.passengers.length-1}`:""}</td>
                ...
                <td style={{padding:"12px 14px",display:"flex",gap:4,flexWrap:"wrap"}} onClick={e=>e.stopPropagation()}>
                  {b.status==="confirmed"&&<>{btn("? Confirm",C.emerald,()=>sendBookingEmail(b._id,"confirmation"),sendingEmail===b._id+"confirmation")}{btn("?? E-Ticket",C.cyan,()=>sendBookingEmail(b._id,"eticket"),sendingEmail===b._id+"eticket")}{btn("? Cancel",C.hot,()=>cancelBooking(b._id))}</>}
                  {b.status==="pending"&&btn("? Cancel",C.hot,()=>cancelBooking(b._id))}
                  {btn("? Custom",C.accentLight,()=>{setEmailTarget(b);setShowEmailModal(true)})}
                </td>
              </tr>
              {/* Expanded row */}
              {isExpanded&&(
                <tr key={b._id+"exp"}><td colSpan={8} style={{padding:0}}>
                  <div style={{padding:"16px 24px",background:"rgba(99,102,241,0.03)",borderBottom:`1px solid ${C.glassBorder}`}}>
                    ...
                    <div style={{display:"flex",gap:8}}>
                      <a href={`/checkin`} target="_blank" style={{padding:"6px 14px",borderRadius:8,background:C.accent+"15",border:`1px solid ${C.accent}25`,color:C.accentLight,fontSize:11,fontWeight:700,textDecoration:"none",cursor:"pointer"}}>?? Check-In Page</a>
                      <a href={`/boarding-pass/${b.bookingReference}`} target="_blank" style={{padding:"6px 14px",borderRadius:8,background:C.cyan+"15",border:`1px solid ${C.cyan}25`,color:C.cyan,fontSize:11,fontWeight:700,textDecoration:"none",cursor:"pointer"}}>?? Boarding Pass</a>
      ...
      {showEmailModal&&emailTarget&&(
        <div style={{position:"fixed",inset:0,background:"rgba(0,0,0,0.7)",display:"flex",alignItems:"center",justifyContent:"center",zIndex:999}} onClick={()=>{setShowEmailModal(false);setEmailMsg("")}}>
          <div style={{background:C.card,border:`1px solid ${C.glassBorder}`,borderRadius:20,padding:32,width:"100%",maxWidth:520,animation:"fadeUp 0.3s ease-out"}} onClick={e=>e.stopPropagation()}>
            ...
            {emailMsg&&<div style={{padding:"8px 14px",marginBottom:14,borderRadius:8,background:emailMsg.startsWith("?")?C.emerald+"12":C.hot+"12",color:emailMsg.startsWith("?")?C.emerald:C.hot,fontSize:12,fontWeight:600}}>{emailMsg}</div>}
