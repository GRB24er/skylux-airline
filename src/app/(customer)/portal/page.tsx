...
export default function CustomerPortal(){
  ...
  const confirmBooking=async()=>{if(!user)return;
      ...
      setBookingLoading(false);return;
        ...
        setBookingLoading(true);try{const bookingBody:any={flightIds:[selectedFlight._id],passengers:passengers.map(p=>({firstName:p.firstName,lastName:p.lastName,email:p.email,dateOfBirth:p.dateOfBirth,nationality:p.nationality,passportNumber:p.passportNumber,passportExpiry:p.passportExpiry,mealPreference:p.mealPreference,specialRequests:p.specialRequests?[p.specialRequests]:[],cabinClass:selectedClass,phone:p.phone})),cabinClass:selectedClass,contactEmail,contactPhone,addOns,paymentMethod:payMethod};if(selectedFlight._duffelRaw){bookingBody.duffelOffer=selectedFlight._duffelRaw}const res=await fetch("/api/bookings/create",{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify(bookingBody)});const data=await res.json();if(data.success){
